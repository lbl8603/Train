...
/**
 * @Author:lubeilin
 * @Date:Created in 14:50 2020/2/8
 * @Modified By:
 */
public class ImageUtils {
     ...
     * 图像二值化处理
     *
     * @param bufferedImage 要处理的图片
     * @throws IOException
     */
    public static BufferedImage binarization(BufferedImage bufferedImage) throws IOException {
        // 获取当前图片的高,宽,ARGB
        int h = bufferedImage.getHeight();
        int w = bufferedImage.getWidth();
        int arr[][] = gray(bufferedImage);
...
//        BufferedImage bufferedImage = new BufferedImage(w, h, BufferedImage.TYPE_BYTE_BINARY);
    ...
    private static int[][] gray(BufferedImage bi){
        int h = bi.getHeight();
        int w = bi.getWidth();
        int arr[][] = new int[w][h];
        // 获取图片每一像素点的灰度值
